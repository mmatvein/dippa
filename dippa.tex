...
The last key factor, interactivity, means that the virtual environment is affected by user actions. Interaction is usually implemented by having positional and rotational tracking combined with some form of controller, such as a wand, discussed more in section \ref{subsection:motion:move}. Nowadays wands can often be left out, as gesture detection systems are becoming sophisticated enough to allow users to input actions with more natural interaction. Section \ref{subsection:motion:gesture} contains a short overview of gesture detection.
