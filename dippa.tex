...
The CAVE Automatic Virtual Environment, or CAVE for short, is one of the most well known VR setups. Introduced by Cruz-Neira et al. \cite{CAVE1, CAVE2}, the original CAVE was a room within a room.

Aalto University has its own semi-CAVE environment, Upponurkka, introduced by Lokki et al. \cite{Upponurkka} which features two projection screens arranged to fit a corner. For each of these screens, two projectors project polarized images, which users can then see as stereo images using regular polarizing 3D glasses. The setup also includes a Kinect camera along with a PlayStation Eye connected to a PlayStation 3. A diagram of the typical Upponurkka setup can be seen in figure \ref{figure:upponurkka}.
...
These values can be inserted into the default perspective projection matrix $P_p$ to get an off-axis projection.

\hl{modifying camera rotation and translation matrices to work in unity}
...
Whereas the default way to calculate the projection of point $p$ would be to apply only the modelview and projection matrices, the method of Lancelle et al. adds an additional matrix $C$ to the equation, resulting in the keystone corrected point $r = C * P * M * p$. Where $P$ and $M$ are the projection and modelview matrices, respectively. The matrix $C$ contains the necessary transformations to correct translation, scaling, shearing, rotation and trapezoid deformation in the final image. These deformations can be corrected by using a matrix that has the ten parameters shown in \eqref{matrix:keystoneC}. In this matrix $i = m$ and $j = n$.
...
C = \begin{pmatrix}
 ...
 This matrix $C$ can be numerically optimized to find a solution that approximates the perfect distortion cancelling matrix. This perfect matrix is the matrix that corrects the display corner point to the user specified new corner point. For the optimization process, Lancelle et al. use the steepest descent algorithm and provide a code example in C, of which we implemented our own C\# version that uses the interface provided by Unity.
