...
Akenine-Möller et al. \cite{RTR3} define the basic perspective projection matrix $P_p$ in equation \eqref{matrix:basicperspective}, where $n$, $f$, $t$, $b$, $r$ and $l$ are the near, far, top, bottom, right and left planes, respectively. This matrix, when applied to a point $p$, transforms $p$ into normalized device coordinates. Finally, to get the final projected point $q$ on the near plane, $p$ will have to be divided by its $w$ coordinate - the fourth component: $q = (p_x/w, p_y/w, p_z/w)$.
