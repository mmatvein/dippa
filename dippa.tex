...
According to Kooima, the following intermediary values are to be calculated in order to get to the final $r$, $l$, $t$ and $b$ values: $v_a$, $v_b$ and $v_c$, the vectors from the eye location to the display corners and d, the distance from the eye to the display. These values are computed with the equations \eqref{equation:perspectiveintermediary}-\eqref{equation:perspectiveintermediarylast}.
