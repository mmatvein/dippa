...
According to Kooima, the following intermediary values are to be calculated in order to get to the final r, l, t and b values: v\textsubscript{a}, v\textsubscript{b} and v\textsubscript{c}, the vectors from the eye location to the display corners and d, the distance from the eye to the display. These values are computed in the fashion shown in equations \eqref{equation:perspectiveintermediary}-\label{equation:perspectiveintermediarylast}.

%\begin{equation}
\begin{align}
\label{equation:perspectiveintermediary}
    v_a& = p_a - p_e, \\
    v_b& = p_b - p_e, \\
    v_c& = p_c - p_e, \\
    d& = -(v_a \cdot v_n) \label{equation:perspectiveintermediarylast}
\end{align}
%\end{equation}
